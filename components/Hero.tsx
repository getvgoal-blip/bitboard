  ...
  return (
    <section className="relative py-6 sm:py-10 overflow-hidden">
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse 90% 50% at 50% 0%, rgba(247, 147, 26, 0.12) 0%, transparent 70%)",
        }}
      />
...
function BTCPriceCard({
  ...
  const flashStyle: React.CSSProperties =
    flash === "up"
      ? { textShadow: "0 0 30px rgba(34,197,94,0.8)", color: "#22c55e", transition: "color 0.15s, text-shadow 0.15s" }
      : flash === "down"
      ? { textShadow: "0 0 30px rgba(239,68,68,0.8)", color: "#ef4444", transition: "color 0.15s, text-shadow 0.15s" }
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: "0 0 40px rgba(247,147,26,0.08), 0 8px 32px rgba(0,0,0,0.4)",
      }}
    >
      ...
      <div className="mb-5">
        {isLoading ? (
          ...
          <>
            <p className="num font-black leading-none" style={{ fontSize: "clamp(44px, 6vw, 64px)", ...flashStyle }}>
      ...
      {!isLoading && btc && (
        <div className="mt-4 pt-3 flex items-center justify-between" style={{ borderTop: "1px solid var(--border-color)" }}>
          <div className="flex items-center gap-1.5">
            <span className="text-[11px] font-semibold" style={{ color: "#22c55e" }}>고</span>
            <span className="text-sm font-bold num text-white">${formatPrice(btc.high24h)}</span>
          </div>
          <div className="h-1.5 flex-1 mx-3 rounded-full overflow-hidden" style={{ background: "#1e1e30" }}>
            <div
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[11px] font-semibold" style={{ color: "#ef4444" }}>저</span>
          </div>
        </div>
      )}
      ...
      <div className="flex items-end gap-0.5 mt-4" style={{ height: "32px" }}>
        {[40, 55, 45, 60, 52, 68, 58, 72, 65, 78, 70, 85].map((h, i) => (
          <div
            ...
            style={{
              height: `${h}%`,
              background: i === 11 ? "#f7931a" : isPositive ? "rgba(34, 197, 94, 0.3)" : "rgba(239, 68, 68, 0.3)",
...
function MarketStatusCard({
  ...
  return (
    <div className="card fade-in" style={{ animationDelay: "0.1s", boxShadow: "0 8px 32px rgba(0,0,0,0.35)" }}>
      <div className="flex items-center justify-between mb-4">
        ...
        {fearGreed ? (
          <div className="flex flex-col items-center px-4 py-3 rounded-xl" style={{ background: "var(--bg-elevated)", border: `1px solid ${fgColor}22` }}>
      ...
      <div className="mb-4">
        <div className="relative h-2.5 rounded-full overflow-hidden mb-1.5"
          style={{ background: "linear-gradient(90deg, #ef4444 0%, #f97316 25%, #eab308 50%, #84cc16 75%, #22c55e 100%)" }}>
          <div className="absolute top-1/2 -translate-y-1/2 w-3.5 h-3.5 rounded-full border-2 border-white shadow-lg transition-all duration-700"
      ...
      <div className="grid grid-cols-2 gap-2">
        {signals.map((sig) => (
          <div key={sig.label} className="flex items-center justify-between px-3 py-2.5 rounded-lg" style={{ background: "var(--bg-elevated)" }}>
            <span className="text-xs" style={{ color: "#64748b" }}>{sig.label}</span>
            <span className="text-xs font-bold" style={{ color: sig.pos === true ? "#22c55e" : sig.pos === false ? "#ef4444" : "#94a3b8" }}>
              {sig.value}
            </span>
          </div>
        ))}
      ...
      {fearGreed && (
        <div className="mt-3 pt-3 flex items-center justify-between" style={{ borderTop: "1px solid var(--border-color)" }}>
          ...
          <span className="text-sm font-bold" style={{ color: fearGreed.value >= fearGreed.previousValue ? "#22c55e" : "#ef4444" }}>
  ...
  return (
    ...
    >
      <div className="absolute top-0 right-0 w-32 h-32 rounded-full pointer-events-none"
        style={{ background: "radial-gradient(circle, rgba(247, 147, 26, 0.18), transparent 70%)", transform: "translate(30%, -30%)" }} />
      <div className="relative">
        <div className="mb-5">
          <span className="badge badge-orange mb-3">🎁 신규 가입 혜택</span>
          <p className="text-lg font-black text-white mt-2 leading-snug">
            거래소 가입하고<br />
            <span style={{ color: "#f7931a" }}>최대 $30,000</span> 받기
          </p>
          <p className="text-xs mt-1.5" style={{ color: "#64748b" }}>리퍼럴 가입 시 수수료 할인 + 보너스</p>
        </div>
        <div className="flex flex-col gap-2 mb-5">
          {exchanges.map((ex) => (
            <div key={ex.name} className="flex items-center justify-between px-3 py-2.5 rounded-xl"
        ...
        <a href="#exchange"
          ...
          style={{ background: "linear-gradient(135deg, #f7931a, #e8830a)", color: "white", fontSize: 15, boxShadow: "0 4px 20px rgba(247,147,26,0.4)" }}
          onMouseEnter={(e) => { e.currentTarget.style.filter = "brightness(1.1)"; e.currentTarget.style.boxShadow = "0 6px 28px rgba(247,147,26,0.55)"; }}
          onMouseLeave={(e) => { e.currentTarget.style.filter = "brightness(1)"; e.currentTarget.style.boxShadow = "0 4px 20px rgba(247,147,26,0.4)"; }}
        >
          혜택 비교하고 가입하기
